...
./target/release/cogtome serve --port 3334  # Start HTTP API server (Axum)
./start-webui.sh                          # One-click start (builds + runs API on 3334 + WebUI on 3333)
...
| `api.rs` | HTTP API server (Axum) — `/health`, `/complexes`, `/api/structures`, `/api/motifs`, `/api/units`, `/run` endpoints. Serves webui from `webui/dist`. CORS enabled. |
...
React + TypeScript + Vite frontend. Ports: 3333 (dev), API proxied to 3334.

| Command | Description |
|---------|-------------|
| `cd webui && npm run dev` | Start dev server with HMR |
| `cd webui && npm run build` | Production build to `webui/dist` |
| `./start-webui.sh` | One-click: build Rust + start API + start WebUI dev |

**Ports**: Frontend dev: 3333, API: 3334 (both configurable)
